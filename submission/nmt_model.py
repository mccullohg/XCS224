...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        # Concatenate for initial decoder state
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        # Construct target sentence tensor
        Y = self.model_embeddings.target(target_padded)
        # Iterate over the temporal history of the tensor to generate observations
        for ystep in torch.split(Y, split_size_or_sections=1):
            Y_t = torch.squeeze(ystep, dim=0)
            Ybar_t = torch.cat((Y_t, o_prev), dim=1)
            dec_state, o_current, __ = self.step(Ybar_t, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks)
            combined_outputs.append(o_current)
            o_prev = o_current
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # Compute attention scores (b, src_len)
        ...
        # Attention vector using batched matrix multiplication 
