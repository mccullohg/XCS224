...
def naive_softmax_loss_and_gradient(center_word_vec,outside_word_idx,outside_vectors,dataset):
  ...
  ### START CODE HERE
  yHat = softmax(np.dot(outside_vectors, center_word_vec))
  loss = -np.log(yHat[outside_word_idx])  # equation 5

  # Code one-hot vector of true distribution
  y = np.zeros(len(outside_vectors))
  y[outside_word_idx] = 1
  
  grad_center_vec = np.dot(outside_vectors.T, yHat-y)
  grad_outside_vecs = np.outer(yHat-y, center_word_vec)
  ### END CODE HERE
...
def skipgram(current_center_word, window_size, outside_words, word2ind, center_word_vectors, outside_vectors, dataset, word2vec_loss_and_gradient=neg_sampling_loss_and_gradient):
  ...
  ### START CODE HERE
  # Parse relevant search indices
  center_word_idx = word2ind[current_center_word]

  # Pseudo code implementation
  for word in outside_words:
     loss_current, gradc, grado = word2vec_loss_and_gradient(center_word_vectors[center_word_idx], word2ind[word], outside_vectors, dataset)
     loss += loss_current
